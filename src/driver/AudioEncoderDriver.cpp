...
namespace driver
{
    ...
    int AudioEncoderDriver::encode(const uint8_t *pcm_data, int data_size, AVPacket &out_pkt)
    {
        ...
        int max_samples = frame_->nb_samples;
        int max_bytes = max_samples * codec_ctx_->channels * bytes_per_sample;
        int copy_bytes = std::min(data_size, max_bytes);

        if (copy_bytes <= 0)
        {
            LOGW("Invalid PCM data size: %d (max: %d)", data_size, max_bytes);
            return -1;
        }
        ...
        if (start_us_ == 0)
        {
            start_us_ = driver::now_us();
        }

        memcpy(frame_->data[0], pcm_data, copy_bytes);
        uint64_t current_us = driver::now_us();
        // 计算相对时间戳（从编码开始）
        uint64_t pts_us = current_us - start_us_;
        frame_->pts = pts_us; // 相对时间戳（微秒）
        ...
        // 设置输出包时间戳
        out_pkt.pts = pts_us;
        out_pkt.dts = pts_us; // DTS通常与PTS相同

        printf("Encoded audio packet - Size: %d bytes, PTS: %lld\n", out_pkt.size, out_pkt.pts);
        return 0;
    }
